...
public class KeyboardPageViewController : PageViewController
{
    ...
    void CleanupTargets()
    {
        var targetsToRemove = _keyboardBehaviors
            .Where(i => !i.TryGetTarget(out var target))
            .ToList();
    ...
    void ProcessPressses(UIPressesData presses, bool isKeyUp = false)
    {
        ...
        var targets = _keyboardBehaviors
            .Select(i => i.TryGetTarget(out var target) ? target : null)
            .Where(i => i != null)
            .ToList();
        ...
        {
            if (isKeyUp)
                registration?.RaiseKeyUp(eventArgs);
            else
                registration?.RaiseKeyDown(eventArgs);
        }
    }
}
