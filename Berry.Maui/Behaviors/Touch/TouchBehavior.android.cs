...
public partial class TouchBehavior
{
    ...
    protected override void OnAttachedTo(VisualElement bindable, AView platformView)
    {
        ...
        accessibilityManager =
            platformView.Context?.GetSystemService(Context.AccessibilityService)
    ...
    private void CreateRipple()
    {
        ...
        var drawable = isAtLeastM && viewGroup is null ? view?.Foreground : view?.Background;
        ...
        if (
            drawable is RippleDrawable rippleDrawable
            && rippleDrawable.GetConstantState() is Drawable.ConstantState constantState
        )
        {
    ...
    private void UpdateRipple(MColor color)
    {
        ...
        {
            ripple.Radius = (int)(
                view?.Context?.Resources?.DisplayMetrics?.Density * NativeAnimationRadius
    ...
    private void ApplyRipple()
    {
        ...
        {
            rippleView = new FrameLayout(
                viewGroup?.Context ?? view?.Context ?? throw new NullReferenceException()
            )
            {
    ...
    private void OnTouchMove(object? sender, AView.TouchEventArgs e)
    {
        ...
        var diffX =
            Math.Abs(e.Event.GetX() - startX)
                / this.view?.Context?.Resources?.DisplayMetrics?.Density
            ?? throw new NullReferenceException();
        var diffY =
            Math.Abs(e.Event.GetY() - startY)
                / this.view?.Context?.Resources?.DisplayMetrics?.Density
            ?? throw new NullReferenceException();
        ...
        var status = viewRect.Contains(screenPointerCoords)
            ? TouchStatus.Started
    ...
    private sealed class AccessibilityListener
        : Java.Lang.Object,
