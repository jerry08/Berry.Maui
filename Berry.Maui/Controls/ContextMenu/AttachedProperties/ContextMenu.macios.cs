...
public static partial class ContextMenu
{
    ...
    static partial void SetupClickCommand(BindableObject bindable)
    {
        ...
        )
        {
            ...
            var gestureRecognizer = new UITapGestureRecognizer(
                () => ExecuteClickCommand(visualElement, visualElement)
            );
    ...
    static partial void DisposeClickCommand(BindableObject bindable)
    {
        if (bindable is VisualElement visualElement)
        {
            ...
            var gestureRecognizer = (UITapGestureRecognizer)
                visualElement.GetValue(TapGestureRecognizerProperty);
    ...
    {
        var children = new UIMenuElement[menu.Children.Count];
        var i = 0;
    ...
    {
        var children = new UIMenuElement[group.Children.Count];
        var i = 0;
    ...
    static UIMenuElement CreateSubMenu(Menu menu)
    {
        ...
        var a = UIMenu.Create(menu.Title, null, UIMenuIdentifier.None, default, children);
    ...
    )
    {
        ...
        else
        {
            ...
            return new UITargetedPreview(
                inst.ToPlatform(visualElement.Handler.MauiContext),
                parameters,
                new UIPreviewTarget(
                    target,
                    new CGPoint(target.Bounds.GetMidX(), target.Bounds.GetMidY())
                )
            );
        }
    }
...
public class ContextMenuInteractionDelegate : UIKit.UIContextMenuInteractionDelegate
{
    ...
    public override UITargetedPreview GetHighlightPreview(
        UIContextMenuInteraction interaction,
        UIContextMenuConfiguration configuration,
    ...
    public override UITargetedPreview? GetPreviewForHighlightingMenu(
        UIContextMenuInteraction interaction,
        UIContextMenuConfiguration configuration
    )
    {
