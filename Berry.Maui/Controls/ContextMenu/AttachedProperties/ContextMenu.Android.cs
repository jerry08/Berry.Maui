...
public static partial class ContextMenu
{
    ...
    public const string PreviewWindowBackgroundColorResource =
        "AndroidContextMenuPreviewWindowBackgroundColor";
    public const string PreviewWindowBackgroundOpacityResource =
        "AndroidContextMenuPreviewWindowBackgroundOpacity";
    public const string ContextMenuBackgroundColorResource =
        "AndroidContextMenuContextMenuBackgroundColor";
    public const string ContextMenuCornerRadiusResource =
        "AndroidContextMenuContextMenuCornerRadius";

    public static readonly BindableProperty ChildElementsProperty = BindableProperty.CreateAttached(
        "ChildElements",
    ...
    public static (int, int) AddRootMenuItem(
        MenuElement item,
        IMenu amenu,
        int rootGroupId,
        int rootId
    ...
    static float DpToPixel(float dp)
    {
        return dp
            * (
                (float)Platform.CurrentActivity.Resources.DisplayMetrics.DensityDpi
                / (float)DisplayMetricsDensity.Default
            );
    }
    ...
    public static void SetIsDestructive(IMenuItem item, Action action)
    {
        if (action.IsDestructive && action.IsEnabled)
        {
            item.SetIconTintList(
                new ColorStateList(
                    new int[][] { new int[] { } },
                    new int[] { Microsoft.Maui.Graphics.Color.FromArgb("#F44336").ToPlatform() }
                )
            );
        }
    }
