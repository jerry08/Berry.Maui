...
internal class ContextMenuBackgroundView : FrameLayout
{
    ...
    public ContextMenuBackgroundView(Context context)
        : base(context) { }

    protected override void OnAttachedToWindow()
    {
        base.OnAttachedToWindow();
        var color = Application.Current.GetResourceOrDefault(
            ContextMenu.PreviewWindowBackgroundColorResource,
            Colors.Gray
        );
        var opacity = Application.Current.GetResourceOrDefault(
            ContextMenu.PreviewWindowBackgroundColorResource,
            .05f
        );
...
{
    public ContextMenuTemplatedPreview(Context context, Preview? preview)
...
internal class ContextMenuPreview : FrameLayout
{
    ...
    public ContextMenuPreview(Context context, Preview? preview)
        : base(context)
    {
        _context = context;
        _preview = preview;
...
{
    public ContextMenuSnapshotPreview(Context context, Bitmap bitmap, Preview? preview)
        : base(context, preview)
    {
...
internal class ContextMenuWindow
{
    ...
    void ShowAfterLayout()
    {
        ...
        _content
            ...
            .WithEndAction(() =>
            {
                _menu.Show(
                    _offsetX,
                    _offsetY + ViewUtils.DpToPx(ContextMenuWindow.ContextMenuSpacing)
                );
            })
            .Start();
    ...
    {
        _backgroundView.Animate().Alpha(1).SetDuration(ShowAnimationDuration).Start();
    }

    void AnimateBackgroundOut()
    {
        _backgroundView.Animate().Alpha(0).SetDuration(DismissAnimationDuration).Start();
    }
...
class Function1Impl<T> : Java.Lang.Object, IFunction1
