...
internal class ContextMenuItemView : LinearLayout
{
    ...
    void Setup()
    {
        ...
        _context.Theme.ResolveAttribute(
            Berry.Maui.Resource.Attribute.selectableItemBackground,
            outValue,
            true
        );
        ...
        {
            Background = _context.Resources.GetDrawable(
                Android.Resource.Drawable.DividerHorizontalBright
            ),
        };
...
internal class ContextMenuViewAdapter : RecyclerView.Adapter
{
    internal class ViewHolder : RecyclerView.ViewHolder, AView.IOnClickListener
    {
        ...
        public ViewHolder(AView itemView)
            : base(itemView)
        {
    ...
    public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
    {
        if (holder is ViewHolder h && h.ItemView is ContextMenuItemView view)
        {
            ...
            if (item.IsVisible)
            {
                ...
                var prevGroupId =
                    position - 1 >= 0 ? _menu.GetItem(position - 1).GroupId : currGroupId;
        ...
        if (itemImpl.HasSubMenu)
        {
            if (sender is ViewHolder holder)
            {
                ...
                SubMenuSelected?.Invoke(
                    this,
                    new SubMenuSelectedEventArgs(holder.ItemView, subMenu)
                );
            }
        }
        else
...
internal class ContextMenuPopup : PopupWindow
{
    ...
    public ContextMenuPopup(AView anchor, SubMenuBuilder subMenu, ContextMenuPopup parent)
        : base(anchor.Context)
    {
        _parent = parent;
    ...
    public ContextMenuPopup(AView anchor, bool closeOnOutsideClick = false)
        : base(anchor.Context)
    {
    ...
    {
        return (
                (int)_anchor.Context.Resources.Configuration.UiMode
                & (int)Android.Content.Res.UiMode.NightMask
            ) == (int)Android.Content.Res.UiMode.NightYes;
    }
    ...
    void Setup(AView anchor, MenuBuilder menu)
    {
        ...
        var backgroundColor = Application.Current.GetResourceOrDefault<Color>(
            ContextMenu.ContextMenuBackgroundColorResource,
            null
        );
        var cornerRadius = Application.Current.GetResourceOrDefault(
            ContextMenu.ContextMenuCornerRadiusResource,
            new CornerRadius(18)
        );
        ...
        s.Paint.Color =
            backgroundColor != null
                ...
                : new Android.Graphics.Color(
                    _anchor.Context.GetColor(
                        IsDarkMode()
                            ? Android.Resource.Color.BackgroundDark
                            : Android.Resource.Color.BackgroundLight
                    )
                );
