...
public static class WindowInsetsExtensions
{
    public static Thickness ToThickness(this WindowInsetsCompat windowInsets)
    {
        ...
        return new Thickness(
            0,
            insets.Top / DeviceDisplay.MainDisplayInfo.Density,
            0,
            insets.Bottom / DeviceDisplay.MainDisplayInfo.Density
        );
    }
}
...
public partial class Insets
{
        ...
        switch (style)
        {
            ...
            default:
                throw new NotSupportedException(
                    $"{nameof(StatusBarStyle)} {style} is not yet supported on Android"
                );
        }
    }
    ...
    static AWindow GetCurrentWindow(Activity activity)
    {
        var window =
            activity.Window
            ?? throw new InvalidOperationException($"{nameof(activity.Window)} cannot be null");
