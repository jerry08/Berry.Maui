...
public class ViewSwitcher : Grid, IDisposable
{
    ...
    private void UpdateSelectedView(int selectedIndex)
    {
        ...
        var previousVisibleViewIndex = -1;
    ...
    private void ShowView(View view, int viewIndex)
    {
        ...
        if (
            Animate
            && view is IAnimatableReveal animatable
            && animatable.Animate
            && view.Opacity == 0
            && _isAttached
        )
        {
            ...
            {
                var fadeTask = localView.FadeTo(1, 500);
                var translateTask = localView.TranslateTo(0, 0, 250, Easing.CubicOut);
