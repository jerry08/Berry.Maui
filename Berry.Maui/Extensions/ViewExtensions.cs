...
public static partial class ViewExtensions
{
    ...
    internal static double ExtractAngleInDegrees(this Matrix4x4 matrix) =>
        ExtractAngleInRadians(matrix) * 180 / Math.PI;
...
#endif
    {
        ...
        {
            var parent = view?.GetParent() as T;
...
#if WINDOWS || ANDROID
    internal static ParentView? FindParent(
        this PlatformView? view,
        Func<ParentView?, bool> searchExpression
    )
    {
...
#if PLATFORM
        if (
            element.Handler is IPlatformViewHandler platformViewHandler
...
#if PLATFORM
    internal static T? FindDescendantView<T>(
        this PlatformView view,
        Func<PlatformView, bool> predicate
    )
